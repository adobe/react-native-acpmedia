...
```
cd sample/ACPMediaSampleApp && npm install && cd ios/ && pod update && cd .. && react-native run-ios
```
...
or
```
cd sample/ACPMediaSampleApp/ && npm install && cd ios/ && pod update
```
...
```
cd sample/ACPMediaSampleApp/ && npm install && react-native run-android
```
